...
Nhiệm vụ của bạn là kiểm tra xem yêu cầu của người dùng có tuân thủ các chính sách nội bộ được cung cấp trong context hay không.

PHẢI TUÂN THỦ CÁC QUY TẮC SAU:
1. Xác định đúng ngoại lệ (Exceptions):
   - Flash Sale: Không được hoàn tiền.
   - Sản phẩm kỹ thuật số/License/Subscription: Không được hoàn tiền.
   - Đã kích hoạt/sử dụng: Không được hoàn tiền.
2. Kiểm tra thời gian (Temporal Scoping):
   - Đơn hàng trước 01/02/2026 áp dụng chính sách v3 (không có trong tài liệu hiện tại).
3. Luôn trả về định dạng JSON chính xác như sau:
{
  "policy_applies": boolean (true nếu hợp lệ, false nếu vi phạm ngoại lệ),
  "policy_name": string (ví dụ: "refund_policy_v4"),
  "exceptions_found": [ 
    {"type": "tên_loại", "rule": "nội dung quy tắc", "source": "tên_file"} 
  ],
  "policy_version_note": string (nếu có lưu ý về phiên bản),
  "explanation": "giải thích ngắn gọn lý do"
}
"""

def _call_llm(messages: List[Dict]) -> str:
    """Gọi LLM (Gemini hoặc OpenAI) để phân tích chính sách."""
    # Thử Gemini trước (ưu tiên trong lab này)
    try:
...
def analyze_policy(task: str, chunks: list) -> dict:
    ...
    if not policy_applies:
        ...
        messages = [
            {"role": "system", "content": POLICY_ANALYST_PROMPT},
        ...
        if llm_response:
            try:
                # Làm sạch response (bỏ code blocks nếu có)
                ...
                print(f"⚠️  LLM Policy parsing failed: {e}.")
                return {
                    "policy_applies": policy_applies,
                    "policy_name": policy_name,
                    "exceptions_found": exceptions_found,
                    "source": sources,
                    "policy_version_note": policy_version_note,
                    "explanation": "Analyzed via rule-based policy check. LLM Policy parsing failed.",
                }  
